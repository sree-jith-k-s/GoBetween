<%-- 
    Document   : NewDriverList
    Created on : May 16, 2021, 3:15:40 PM
    Author     : Pro-TECH
--%>
...
<html lang="en" >
    ...
    <%
        ...
        if (request.getParameter("btn_rej") != null) {
            ...
            String insQry = "update tbl_driver_request set request_status = 'Rejected' where request_id='" + request.getParameter("btn_rej") + "'";
            if (con.executeCommand(insQry)) {

                String del = "delete from tbl_company_driver where driver_id='" + request.getParameter("id") + "'";
                if (con.executeCommand(del)) {
    %> 
    <script type="text/javascript">
        alert("Rejected Successfully..");
        setTimeout(function() {
            window.location.href = 'NewDriverList.jsp'
        }, 100);
    </script>
    <%
                }
    ...
    %>
    <body>
        <h1 align="center" id="heading">Driver List</h1>
        <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
            ...
            <tbody>
                <%                   int i = 0;
                    String selQry = "select * from tbl_company_driver cd inner join tbl_driver_request dr on dr.driver_id=cd.driver_id inner join tbl_driver d on d.driver_id=cd.driver_id inner join tbl_location l on l.location_id=d.location_id inner join tbl_district dis on dis.district_id=l.district_id inner join tbl_state s on s.state_id=dis.state_id inner join tbl_company c on c.company_id=cd.company_id where cd.company_id='" + session.getAttribute("cid") + "'";
                ...
                <tr>
                    <td><%=i%></td>
                    <td><img style="border-radius: 50%" src="../Assets/Files/DriverPhoto/<%=rs.getString("driver_photo")%>" width="80" height="80"></td>
                    <td><%=rs.getString("driver_name")%></td>
                    <td><a href="../Assets/Files/DriverProof/<%=rs.getString("driver_proof")%>" download>Download</a></td>
                    <td><%=rs.getString("state_name")%></td>
                    <td><%=rs.getString("district_name")%></td>
                    <td><%=rs.getString("location_name")%></td>
                    <td><%=rs.getString("driver_primary_contact")%></td>
                    <td><a href="NewDriverList.jsp?btn_rej=<%=rs.getString("request_id")%>&id=<%=rs.getString("driver_id")%>">Reject</a></td>
                </tr>
                <%
                    }
